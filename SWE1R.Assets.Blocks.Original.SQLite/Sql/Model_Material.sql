select
  ...
  printf('0x%x', Model_Material.P_Properties) as m_p_p_x,
  ...
  printf('0x%x', Model_MaterialTexture.P_Child3) as mt_p_c3,
  printf('0x%x', Model_MaterialTexture.P_Child4) as mt_p_c4,
  printf('0x%x', Model_MaterialTexture.IdField) as mt_id_x,
  ...
  printf('0x%x', Model_MaterialProperties.Ints_6_0) as mtp_i60,
  printf('0x%x', Model_MaterialProperties.Ints_6_1) as mtp_i61,
  printf('0x%x', Model_MaterialProperties.Ints_e_0) as mtp_ie0,
  printf('0x%x', Model_MaterialProperties.Ints_e_1) as mtp_ie1,
...
LEFT JOIN BlockItemValue on
  BlockItemValue.Id = Model_Material.Model AND 
...
LEFT JOIN Model_MaterialProperties on 
  ...
  Model_MaterialProperties.Model = Model_Material.Model
where 
  (mt_w >= 64 or mt_h >= 64) 
  and mt_b_c = 2 and mt_b_d = 1
